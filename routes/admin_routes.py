...
async def obter_produto(id_produto: int = Path(..., title="Id do Produto", ge=1)):
    ...
    pb = ProblemDetailsDto("int", f"O produto com id <b>{id_produto}</b> não foi encontrado.", "value_not_found", ["body", "id_produto"])
    return JSONResponse(pb.to_dict(), status_code=404)
...
async def alterar_produto(inputDto: AlterarProdutoDTO):
    ...
    pb = ProblemDetailsDto("int", f"O produto com id {inputDto.id} não foi encontrado.", "value_not_found", ["body", "id_produto"])
    return JSONResponse(pb.to_dict(), status_code=404)
